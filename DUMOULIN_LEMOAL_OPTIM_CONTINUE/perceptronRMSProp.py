...
#Ce perceptron implémente l'algorithme de descente de gradfient avec Root Mean Squared Prop (RMS Prop)
class PerceptronRMS:
        ...
        for i in range (len(self.weights)):
            ...
            if (grad != 0):
                if self.exponential_mean[i] == 0:
                    self.exponential_mean[i] = grad**2
                else:
                    self.exponential_mean[i] = beta*self.exponential_mean[i] + (1 - beta)*(grad**2)
    
                if (objectif == 0):      #Lorsque objectif == 0, on met un facteur 1.5 à la modification puisqu'on essaie d'apprendre plus des rares cas.
                    self.weights[i] = self.weights[i] - 1.5 * lambdda * (grad/math.sqrt(self.exponential_mean[i]))
                else:
                    self.weights[i] = self.weights[i] - lambdda * (grad/math.sqrt(self.exponential_mean[i]))
